...
SCORECARD = ROOT / "reports" / "official-isolated-v68-frozen-public-400.json"
PNG = ROOT / "reports" / "v68-game-level-matrix.png"
SVG = ROOT / "reports" / "v68-game-level-matrix.svg"
...
def render(rows: tuple[GameLevels, ...]) -> None:
    ...
    plt.rcParams.update(
        {
            ...
            "svg.hashsalt": "reflector-v68-game-level-matrix",
        }
    )
    ...
    axis.set_title(
        "Reflector v68 · Public game × level completion",
        fontsize=18,
        color="#24324a",
        pad=22,
    )
    axis.text(
        0.5,
        1.012,
        "28 of 183 levels completed · official local score 9.6840195267 / 100",
    ...
    axis.set_xticks(range(max_levels), labels=range(1, max_levels + 1))
    axis.set_yticks(
        range(len(rows)),
