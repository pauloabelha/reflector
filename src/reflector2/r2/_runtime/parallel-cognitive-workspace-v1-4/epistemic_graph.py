...
def ingest_structured_criticism(
    ...
    *,
    ...
    derivation_id = None if payload is None else payload.get("derivation_id")
    if derivation_id is not None:
        derivation = _objects(state).get(str(derivation_id))
        if derivation is None or derivation.kind != "qwen_derivation" or derivation.created_by != "qwen":
            raise EpistemicGraphError("criticism derivation must be a Qwen derivation")
        if str(target_id) not in derivation.dependency_ids:
            raise EpistemicGraphError("criticism derivation does not derive the target")
    dependencies = tuple(sorted(set((
        str(target_id),
        *(str(item) for item in basis_ids),
        *(() if derivation_id is None else (str(derivation_id),)),
    ))))
    next_state, event, object_id = _ingest_object(
        state,
