...
def test_official_swarm_runs_reflector_to_clean_termination(
    ...
    fixture = (Path(__file__).parents[1] / "fixtures" / "official_toolkit").resolve()
...
@pytest.mark.integration
def test_official_swarm_can_disable_development_recording(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    fixture = (Path(__file__).parents[1] / "fixtures" / "official_toolkit").resolve()
    ...
    monkeypatch.setenv("OPERATION_MODE", "offline")
    monkeypatch.setenv("ENVIRONMENTS_DIR", str(fixture))
    monkeypatch.setenv("RECORDINGS_DIR", str(recordings))
