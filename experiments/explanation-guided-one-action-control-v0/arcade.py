...
render = function(){
  ...
  const summary=`<div class=r2-feedback><div class=r2-feedback-title>R2 FEEDBACK · READ BY NEXT SEMANTIC QWEN</div><div><span>VERB</span> ${esc(active.verb||'none')} · ${esc(active.epistemic_status||'open')}</div><div><span>POTENTIAL</span> ${esc(potential.observable||'open')} = ${esc(potential.value??'?')} → ${esc(potential.terminal_class||'open')}</div><div><span>MECHANISM</span> ${mechanism.action==null?'open':`${actionBadge(mechanism.action)} · expected progress ${esc(mechanism.expected_progress??'?')}`}</div><div><span>SETTLEMENT</span> ${esc(settlement.adjudication||'none')}</div><div><span>CONTEXT</span> ${Number((feedback.salient_structural_bindings||[]).length)} structural bindings · ${Number((feedback.open_shadows||[]).length)} open shadows</div></div>`;
