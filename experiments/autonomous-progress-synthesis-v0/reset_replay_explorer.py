...
def search(world:World,*,action_budget:int=400,max_depth:int=12,max_states:int=256)->SearchResult:
    ...
    """
    if action_budget<1 or max_depth<1 or max_states<2:raise ExplorationError("invalid search bound")
    ...
    queue=deque([()]);keys={root_key:()};edges=[];spent=0;deepest=0
    while queue and spent<action_budget and len(keys)<max_states:
        ...
        for action_index in range(10_000):
            ...
            if done:return SearchResult(True,path,spent,resets,len(keys)+int(target not in keys),deepest,tuple(edges),"environment-completion")
            if not dead and target not in keys and len(keys)<max_states:
                keys[target]=path;queue.append(path)
