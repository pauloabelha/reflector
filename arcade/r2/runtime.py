...
class LiveRuntime:
    ...
    def set_schema_observer(self, observer: Any) -> None:
        """Attach the R2.2 frame-local epistemic fitting layer."""
    ...
    def observe_schemas(self, frame: list[list[int]], turn: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            return {"engine": "R2.2", "turn": int(turn), "levels": [], "error": f"{type(error).__name__}: {error}"}
    ...
    def update(self, **values: Any) -> None:
        # Frame publication and R2.2 fitting are one atomic semantic event.
